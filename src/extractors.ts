...
export type ExtractionOptions = {
  /**
   * Regex applied to each line of the commit message (after the squash dump
   * is stripped). The first capture group must wrap the issue-key portion
   * (one or more IDs, optionally separated by commas / "and" / "&"). Anchor
   * with `^` to match only at the start of a line. Example: `^\[(.+?)\]` for
   * PR titles like `[LIN-123] Add foo`.
   */
